...
struct DisputeStateImporter<BlockNumber> {
	...
	now: BlockNumber,
	new_participants: bitvec::vec::BitVec<BitOrderLsb0, u8>,
	/// Determines if the dispute was `confirmed` /w `2/3` based on the previous state
	/// but also if there was threshold of 1/3 in either way, which allows to conclude the disputes.
